          ...
          <div className='overflow-hidden py-4'>
            ...
            >
              {t.members.map((member) => (
                ...
                >
                  <div className='group relative w-full max-w-[280px] sm:max-w-[320px] rounded-3xl border border-slate-200/90 dark:border-white/10 bg-white/90 dark:bg-[#0e1022]/90 shadow-xl hover:shadow-2xl hover:border-violet-500/40 dark:hover:border-violet-500/40 transition-all duration-300 backdrop-blur-xl flex flex-col items-center text-center overflow-hidden min-h-[440px] sm:min-h-[480px]'>

                    {/* Image covering top like square */}
                    <div className='relative w-full aspect-square overflow-hidden bg-slate-100 dark:bg-slate-800 shrink-0'>
                      <Image
                        src={member.image}
                        alt={member.name}
                        fill
                    ...
                    {/* Card Content Body */}
                    <div className='p-5 sm:p-6 flex flex-col items-center flex-1 w-full justify-between'>
                      <div className='flex flex-col items-center w-full'>
                        {/* Name & Job Role */}
                        <h3 className='text-lg sm:text-xl font-black text-slate-900 dark:text-white tracking-tight mb-1 line-clamp-1'>
                          {member.name}
                        </h3>
                        <div className='inline-block rounded-full bg-violet-500/10 dark:bg-violet-500/20 border border-violet-500/20 px-3 py-0.5 text-xs font-extrabold text-violet-700 dark:text-violet-300 mb-3'>
                          {member.role}
                        </div>
                      ...
                      <div className='flex items-center gap-3 pt-3 border-t border-slate-200/80 dark:border-white/10 w-full justify-center mt-auto'>
                        <a
                          href={member.github}
                          target='_blank'
                          rel='noopener noreferrer'
                          aria-label={`${member.name}'s GitHub`}
                          className='flex h-9 w-9 items-center justify-center rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:text-white hover:bg-slate-900 dark:hover:bg-violet-600 hover:border-slate-900 dark:hover:border-violet-600 transition-all duration-200 cursor-pointer shadow-2xs'
                        >
                        ...
                        <a
                          ...
                          className='flex h-9 w-9 items-center justify-center rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:text-white hover:bg-[#0077b5] hover:border-[#0077b5] transition-all duration-200 cursor-pointer shadow-2xs'
                        >
                          <svg className='h-4.5 w-4.5 fill-current' viewBox='0 0 24 24'>
